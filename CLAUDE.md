...
| 프로젝트 | Tailwind 버전 | 비고 |
|---------|-------------|------|
| Upbox-2.0-Front-Boilerplate | v3 | tailwind-preset 사용 |
| Upbox-2.0-Customer-Report | v3 | tailwind-preset 사용 |
| Upbox-2.0-Front-Application | v4 (`@theme`) | 자체 `@theme`에 색상 정의, `@source`로 dist 스캔 |

### Tailwind v4 컨슈머 주의사항

Tailwind v4는 `@theme` 블록에 색상이 등록되어야 유틸리티가 생성됨. upbox-ui의 CSS 변수만으로는 Tailwind v4 유틸리티가 자동 생성되지 않으므로, `@theme`에 `--color-grey-*` 등을 직접 정의해야 함.
