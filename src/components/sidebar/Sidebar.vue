...
const { isMobile, disableMobile, state, openMobile, setOpenMobile } = useSidebar()
</script>

<template>
  <div
    v-if="collapsible === 'none'"
    :class="cn('flex h-full w-(--sidebar-width) flex-col bg-grey-10 text-grey-100', props.class)"
    v-bind="$attrs"
  >
    <slot />
  </div>
  ...
  <div
    v-else :class="cn('group peer', disableMobile ? 'block' : 'hidden md:block')"
    :data-state="state"
